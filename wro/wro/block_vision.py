...
if __name__ == "__main__":
    # --- Replace camera feed with a screenshot ---
    screenshot_path = r"WRO-FE-RocheReapers\wro\img\3test_screenshot_02.04.2025.png"  # Replace with the actual path to your screenshot image
    try:
        frame = cv2.imread(screenshot_path)
        if frame is None:
            print(f"Error: Could not open or find the image at {screenshot_path}")
            exit()
        ...
        if nearest_block_data is not None:
            ...
            try:
                if processed_frame is not None:
                    ...
                    if "bounding_box" in nearest_block_data[3]:  # Access bounding_box from processed_frame (which contains found_blocks)
                        for block in recognize_blocks(frame.copy())[1]:
                            if block["center"] == center:
                                bx, by, bw, bh = block["bounding_box"]
                                break
                    elif "bounding_box" in nearest_block_data[0]: # This won't work as nearest_block_data[0] is color
                        bx, by, bw, bh = nearest_block_data[0]["bounding_box"]
                    else:
                        for block in recognize_blocks(frame.copy())[1]:
                            if block["center"] == center:
                                bx, by, bw, bh = block["bounding_box"]
                    ...
                    if bx is not None:
                        ...
                        cv2.imshow("Block Recognition from Screenshot", processed_frame)

            except Exception as e:
                print(f"Error displaying image: {e}")
        else:
            print("No nearest block data available.")
            cv2.imshow("Block Recognition from Screenshot", frame)

        cv2.waitKey(0)  # Wait until a key is pressed
        cv2.destroyAllWindows()

    except Exception as e:
        print(f"An error occurred in the main section: {e}")